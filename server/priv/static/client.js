...
class PullRequest extends CustomType {
  constructor(number, title, author, url, created_at, review_decision, draft) {
    super();
    this.number = number;
    this.title = title;
    this.author = author;
...
class PrDetail extends CustomType {
  constructor(number, title, author, url, body, files, diff2) {
    super();
    this.number = number;
    this.title = title;
    this.author = author;
          ...
          return field("created_at", string2, (created_at) => {
            return field("review_decision", string2, (review_decision) => {
              return field("draft", bool, (draft) => {
                return success(new PullRequest(number, title, author, url, created_at, review_decision, draft));
              });
            });
          });
        });
...
function pull_request_list_decoder() {
  return list2(pull_request_decoder());
}
          ...
          return field("body", string2, (body) => {
            return field("files", list2(pr_file_decoder()), (files) => {
              return field("diff", string2, (diff2) => {
                return success(new PrDetail(number, title, author, url, body, files, diff2));
              });
            });
          });
        });
...
class Model extends CustomType {
  constructor(repos, active_repo, prs, selected_pr, loading, view) {
    super();
    this.repos = repos;
...
function fetch_prs(repo) {
  return get3("/api/prs?repo=" + repo, expect_json(pull_request_list_decoder(), (var0) => {
...
function review_badge(decision) {
  let _block;
  if (decision === "APPROVED") {
    _block = ["#d4edda", "#155724"];
  } else if (decision === "CHANGES_REQUESTED") {
    _block = ["#f8d7da", "#721c24"];
  } else if (decision === "REVIEW_REQUIRED") {
    _block = ["#fff3cd", "#856404"];
  } else {
    _block = ["#e2e3e5", "#383d41"];
  }
  let $ = _block;
  let bg;
  let fg;
...
function pr_row(pull_request) {
  return tr(toList([
    ...
    td(toList([styles(toList([["padding", "0.75rem 1rem"]]))]), toList([text3(pull_request.created_at)])),
    td(toList([styles(toList([["padding", "0.75rem 1rem"]]))]), toList([review_badge(pull_request.review_decision)]))
  ]));
}
...
function file_row(file) {
  return div(toList([
    ...
    span(toList([]), toList([text3(file.path)])),
    span(toList([]), toList([
      span(toList([styles(toList([["color", "#22863a"]]))]), toList([text3("+" + to_string(file.additions))])),
      text3(" / "),
      span(toList([styles(toList([["color", "#cb2431"]]))]), toList([text3("-" + to_string(file.deletions))]))
    ]))
  ]));
}
function file_list(files) {
  return div(toList([styles(toList([["margin-bottom", "1.5rem"]]))]), map2(files, file_row));
}
function view2(model) {
  ...
  if ($ instanceof Some) {
    ...
    return div(toList([
      ...
      back_button(),
      h1(toList([
        styles(toList([["color", "#1a1a2e"], ["margin-bottom", "0.5rem"]]))
      ]), toList([text3(detail.title)])),
      p(toList([
        styles(toList([["color", "#666"], ["margin-bottom", "1.5rem"]]))
      ]), toList([text3("by " + detail.author)])),
      h2(toList([styles(toList([["margin-bottom", "0.5rem"]]))]), toList([text3("Description")])),
      pre(toList([
        styles(toList([
          ["background", "#f5f5f5"],
          ["padding", "1rem"],
      ...
      ]), toList([text3(detail.body)])),
      h2(toList([styles(toList([["margin-bottom", "0.5rem"]]))]), toList([
        text3("Files Changed (" + to_string(length(detail.files)) + ")")
      ])),
      ...
      div(toList([
        ...
        h3(toList([styles(toList([["margin-bottom", "0.5rem"]]))]), toList([text3("AI Analysis")])),
        p(toList([styles(toList([["color", "#555"]]))]), toList([text3("AI analysis coming soon...")]))
      ]))
    ]));
  } else {
...
function init(_) {
  ...
  return [
    new Model(toList([default_repo]), default_repo, toList([]), new None, true, new Dashboard),
...
function update2(model, msg) {
  if (msg instanceof GotPrs) {
    ...
    if ($ instanceof Ok) {
      let prs = $[0];
      return [
        new Model(model.repos, model.active_repo, prs, model.selected_pr, false, model.view),
        none()
      ];
    } else {
      return [
        new Model(model.repos, model.active_repo, model.prs, model.selected_pr, false, model.view),
        none()
      ];
    }
    ...
    if ($ instanceof Ok) {
      let detail = $[0];
      return [
        new Model(model.repos, model.active_repo, model.prs, new Some(detail), false, model.view),
        none()
      ];
    } else {
      return [
    ...
    let number = msg[0];
    return [
      new Model(model.repos, model.active_repo, model.prs, model.selected_pr, true, new PrReview),
      fetch_pr_detail(model.active_repo, number)
    ];
  } else if (msg instanceof SetRepo) {
    let repo = msg[0];
    return [
      new Model(model.repos, repo, model.prs, model.selected_pr, model.loading, model.view),
      none()
    ];
  } else if (msg instanceof BackToDashboard) {
    return [
      new Model(model.repos, model.active_repo, model.prs, new None, model.loading, new Dashboard),
      none()
    ];
  } else {
    return [
      new Model(model.repos, model.active_repo, toList([]), model.selected_pr, true, model.view),
...
function main() {
  let app = application(init, update2, view3);
  let $ = start4(app, "#app", undefined);
  if (!($ instanceof Ok)) {
    throw makeError("let_assert", FILEPATH, "client", 15, "main", "Pattern match failed, no pattern matched the value.", { value: $, start: 389, end: 438, pattern_start: 400, pattern_end: 405 });
  }
  return;
}
